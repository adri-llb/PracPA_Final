...
public class EstadoSalas {
   ...
   public void entrarVacunacionPaciente(Paciente paciente) throws IOException{
       c_vac_pacientes.lock();
        ...
        try{
            ...
            interfaz.getjTextFieldPacienteRecepcion().setText("");
   ...
   public void vacunar(int puesto) throws InterruptedException, IOException{
       ...
       try{
           ...
           sanitario =  vacunacion_sanitarios[puesto];
            paciente = vacunacion_pacientes[puesto]; 
   ...
   public void siReaccion(Paciente paciente) throws InterruptedException, IOException{
       if(paciente.getProb_reaccion()<porcentaje_reaccion){
           c_reacciones.lock();
           try {
               String contenido = (paciente.getNombre()+"*");
               jTextField_puestos_observacion[paciente.getPuesto_observacion()].setText(contenido);
   ...
   public int getPuestoLibreObs(){//si la sala de observacion esta llena devuelve  100, de lo contrario devuelve el puesto de observacion
       ...
       for (int i = random; i < observacion_pacientes.length+random; i++) {
                int num = i%19;
                if(observacion_pacientes[num]== null){
                puesto = num;
                }  
           }
       return puesto;
